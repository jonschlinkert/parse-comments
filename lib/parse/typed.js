        ...
        switch (prev.type) {
          ...
          case 'FunctionType':
            if (prev.key) {
              ...
              var param = new Param(prev.key, val);
            ...
            var param = new Expression(val, null);
            prev.params.push(param);
...
// var ast = parse('foo"bar"baz');
// var ast = parse('({"foo": bar, baz: qux} | {"one": two})');
// var ast = parse('({ok: true, data: string} | {ok: false, error: Error})');
// var ast = parse('string|array|function');
